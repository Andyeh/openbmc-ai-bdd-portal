...
docker:
  image:   "openbmc/ast2700-robot-qemu:latest"   # Robot 測試容器
...
從 `robot_script_dir/test_lists/` 自動掃描 `.yaml` 定義的測試套件。點擊卡片選取後按 **▶ Stream Run** 即時執行並串流日誌。
...
2. 選擇 CI 套件或手動挑選測試案例
3. 按 **▶ Stream Run** 啟動，切換至「📡 Live Log」觀看即時輸出
