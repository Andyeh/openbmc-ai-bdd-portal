...
設定 QEMU 參數、選擇 Preset、啟動模擬器，右側即時顯示 Serial Console 互動終端。

![QEMU 操作區](docs/images/openbmc-ai-bdd-portal_qemu-operation-area.png)

### Robot Framework 測試中心 — CI 預設套件

從預掃描的 CI 測試套件卡片中快速選取，一鍵組合多個套件執行，下方即時預覽組裝後的 Robot 指令。

![Robot 測試 — CI 預設套件](docs/images/openbmc-ai-bdd-portal_robot-test-area_default-test-suite.png)

### Robot Framework 測試中心 — 手動挑選測試

依類別標籤篩選或搜尋 `.robot` 測試案例，勾選個別測試後執行，支援精確指定測試名稱。

![Robot 測試 — 手動挑選](docs/images/openbmc-ai-bdd-portal_robot-test-area_manual-selection-area.png)

### Robot Framework 測試中心 — Live Log

測試執行期間即時串流 Robot Framework 輸出日誌，可直接觀察測試進度與結果。

![Robot 測試 — Live Log](docs/images/openbmc-ai-bdd-portal_robot-test-area_live-log.png)

### 測試報告

自動彙整歷次測試結果，顯示通過/失敗統計與執行時間，可直接開啟 HTML Report、Full Log 或 Allure 報告。

![測試報告](docs/images/openbmc-ai-bdd-portal_report-area.png)
